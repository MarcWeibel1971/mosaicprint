...
import { Menu, X, Settings, LogIn, LogOut, FolderOpen, User } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-cream-100/95 backdrop-blur-sm border-b border-cream-300">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {user ? (
              <>
                <Link
                  to="/projekte"
                  className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-coral-500 transition-colors px-2 py-1.5 rounded-lg hover:bg-cream-200"
                >
      ...
      <div
        className={`md:hidden border-t border-cream-300 bg-cream-100 px-6 overflow-hidden transition-all duration-300 ease-in-out ${
          menuOpen ? "max-h-[28rem] py-4 opacity-100" : "max-h-0 py-0 opacity-0"
        }`}
      >
          ...
          {user && (
            <Link
              ...
              className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                isActive("/projekte") ? "bg-coral-50 text-coral-700" : "text-gray-700 hover:bg-cream-200"
              }`}
            >
          ...
          <div className="pt-3 border-t border-cream-300 space-y-2">
            ...
            {user ? (
              ...
              <Link
                to="/login"
                onClick={() => setMenuOpen(false)}
                className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-coral-600 hover:bg-coral-50 transition-colors"
              >
